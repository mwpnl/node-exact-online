...
var services = [
  'CRM', 'Logistics', 'SalesInvoice', 'Sys', 'Financial'
];
...
var Client = function(options) {
  ...
  var defaults = {
    ...
    passToken: function (acces_token) { 
       if (self.options.debug) {
          console.log(self.options.prefixDebug + 'Client - The default passToken function has been called. Please provide your own passToken function in order to save tokens.');
        }},
    startToken: {},
    redirectUri: null
  };
  ...
  this.oauth = {
    ...
    // When initializing the client, check is there's a startToken. This will be used in order to pre-authenticate if possible
  if (this.options.startToken) {
    if (this.options.debug)
      console.log(this.options.prefixDebug + 'Client - startToken present');
      self.oauth.authorized = true;
      self.oauth.token = this.options.startToken;
      self.oauth.refreshToken = this.options.startToken.refresh_token;
      self.oauth.expires = Date.now();
  }
  ...
  services.forEach(function(service) {
    try {
      ...
      console.log('toegevoegd service' );
    } catch(e) {
...
Client.prototype.authorize = function(code, callback) {
  ...
  this.token(code, 'authorization_code', this.options.redirectUri, function(err, token) {
    if(self.options.debug) {
      console.log(self.options.prefixDebug + '.authorize - Retreived token: ' + JSON.stringify(token));
    }

    self.oauth.authorized = true;
    self.oauth.token = token;
    self.oauth.refreshToken = token.refresh_token;
    self.options.passToken(JSON.stringify(self.oauth.token));
