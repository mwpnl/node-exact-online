 ...
 * @param {Function} callback gets called after request is complete
 */
CRM.prototype.getContact = function (contactId, division, callback) {
  var params = {
    $filter: 'ID eq guid\'' + contactId + '\''
  };
  ...
  this.client.sendRequest('/v1/' + division + '/crm/Contacts', 'GET', params, null, callback);
};

/**
 * Alter a contact
 * @param  {String}   contactId Contact ID
 * @param  {String}   contactData  JSON Object with content saved
 * @param  {Integer}  division  Division ID 
 * @param  {Function} callback  Callback
 */
CRM.prototype.saveContact = function (contactId, userData, division, callback) {
  if (typeof division === 'function') {
    callback = division;
    division = this.client.division;
  }
  
  this.client.sendRequest('/v1/' + division + '/crm/Contacts(guid\'' + contactId + '\')', 'PUT', null, userData, callback);
};
 ...
 * @param {Function} callback gets called after request is complete
 */
CRM.prototype.getDirectDebitMandateFromUser = function(userId, division, callback) {
  var params = {
    $filter: 'Account eq guid\''+ userId +'\'',
    $top: 1
  };
 ...
 * @param  {Function} callback   Callback
 */
CRM.prototype.getDocumentAttachment = function(documentId, division, callback) {
  var params = {
    $filter: 'ID eq guid\''+ documentId +'\''
  };
